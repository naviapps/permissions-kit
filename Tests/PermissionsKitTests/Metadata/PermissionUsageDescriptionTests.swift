...
final class PermissionUsageDescriptionTests: XCTestCase {
  ...
  func testMissingUsageDescriptionsReturnsOnlyMissingKeysForCustomPermissions() throws {
    ...
    let custom = try makeCustomPermission(usageDescriptionKeys: [.camera, .microphone])

    XCTAssertEqual(custom.missingUsageDescriptions(in: bundle), [.microphone])
  }

  func testMissingUsageDescriptionsRejectsEmptyOrNonStringValues() throws {
    let bundle = try makeBundle(info: [
      UsageDescriptionKey.camera.rawValue: "  \n",
      UsageDescriptionKey.microphone.rawValue: true,
    ])

    XCTAssertEqual(PermissionType.camera.missingUsageDescriptions(in: bundle), [.camera])
    XCTAssertEqual(PermissionType.microphone.missingUsageDescriptions(in: bundle), [.microphone])
  }
  ...
  private func makeCustomPermission(usageDescriptionKeys: [UsageDescriptionKey]) throws
    -> PermissionType
  {
